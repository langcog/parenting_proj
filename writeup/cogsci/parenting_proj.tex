...
(\(\beta = 0.41\), \emph{SE} = .19, \emph{p} = .03), marginally higher
scores for Affection and Attachment (\(\beta = 0.34\), \emph{SE} = .23,
\emph{p} = .14), whereas gender had no effect on Rules and Respect
scores. The model including parenthood status revealed that parents had
higher scores on the Affection and Attachment subscale compared to
non-parents, (\(\beta = 0.35\), \emph{SE} = .13, \emph{p} = .006),
whereas parenthood status had no effect on Early Learning or Rules and
Respect scores. These differences provide some intuitive support for the
notion that subscale scores tap meaningful constructs that vary across
groups.
...
accuracy for the remaining sample was 0.67 (\emph{SD} = 0.24) for
control recall, 0.83 (\emph{SD} = 0.19) for target recall, and 0.87
(\emph{SD} = 0.22) for target generalization.

\begin{CodeChunk}
\begin{figure*}[t]

{\centering \includegraphics{figs/relation-1} 
