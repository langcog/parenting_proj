...
Do behaviors within categories hang together? This is only important if we use mean scores within these categories for subsequent analyses.
...
Conclusions- Early Learning and Rules and Respect items hang together pretty well (alphas = .70) but Affection and Attachment items are not as great (alpha = .55). Dropping "sleep in the same bed" would help. Also, "hug and kiss" is close to ceiling with most people reporting "multiple times every day."

#Do parenting attitudes predict parenting behaviors?
...
Do demographic variables predict different mean frequencies of individual behaviors?
...
#for AA behaviors 
model <- summary(lmer(rating ~ AA + RR + EL + behave_age + 
                (1|sid) + 
                (1|short_sent), 
              data = d_aa))
summary(model)
