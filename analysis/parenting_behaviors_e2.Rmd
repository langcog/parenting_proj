...
mplot_q <- dq %>% 
  mutate(rating_paq = rating, paq_item = short_sent)%>%
  select(sid, paq_item, category_paq, rating_paq)
  
mplot_b <- db %>% 
  mutate(rating_bev = rating)%>%
  select(sid, short_sent, category_bev, rating_bev)

mplot_b$bev_bin[mplot_b$rating_bev %in% 1:4] <- "low"
mplot_b$bev_bin[mplot_b$rating_bev %in% 5:6] <- "high"

mplot_b <- mplot_b %>%
  select(sid, short_sent, bev_bin)

mplot_q$rating_paq <- as.numeric(mplot_q$rating_paq)

mplot <- mplot_q %>%
  left_join(mplot_b)%>%
  filter(!is.na(bev_bin))%>%
  group_by(short_sent, bev_bin, category_paq)%>%
  multi_boot_standard(col = "rating_paq", na.rm = TRUE) 

ggplot(mplot, aes(short_sent, mean, fill=bev_bin)) +
  geom_bar(stat="identity", position = "dodge") + 
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), 
             position = position_dodge(width = .9)) + 
  scale_fill_solarized()+
  theme(axis.text.x = element_text(angle = 90, hjust = .5, vjust = .5))+ 
  facet_grid(~ category_paq) 
```
