...
qplot(factor, item, fill=loading, geom="tile", data = loadings) + 
  scale_fill_continuous(low="#000000", high="#FFFFFF") + facet_grid( category ~ ., scales = "free_y")

qplot(factor, item, fill=loading > .35, geom="tile", data = loadings) + 
  scale_fill_solarized() + facet_grid( category ~ ., scales = "free_y")
```
...
## Factors by SES
```{r}
a.factor.names <- c("Active Learning Affection 1", "Active Learning Affection 2", "Rules Respect 1", "Rules Respect 2", "Affection")

mf <- a.scores %>% 
  gather(factor, score, starts_with("Factor")) %>%
  filter(ses != "") %>%
  mutate(ses = str_replace_all(ses,"1|2|3","low"),
         ses = str_replace_all(ses,"4|5|6","med"),
         ses = str_replace_all(ses,"7|8|9","high"),
         factor.num = as.numeric(str_replace(factor,"Factor","")),
         factor.name = a.factor.names[factor.num]) %>%
  group_by(ses, factor.name) %>%
  multi_boot_standard(col = "score") 

mf$ses <- factor(mf$ses, levels = c("low","med","high"))

qplot(factor.name, mean, fill=ses, 
      geom = "bar", stat = "identity", position = "dodge", 
      data=mf) + 
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), 
             position = position_dodge(width = .9))
