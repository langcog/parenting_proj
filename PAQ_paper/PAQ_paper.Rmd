...
#Elapsed Time: 340.339 seconds (Warm-up)
#               213.377 seconds (Sampling)
#               553.717 seconds (Total)
...
el_demo_mod <- brm(rating ~ parent_age + ethnicity + parent_ed_years + num_kids + parent_gender +
 ...
 # Elapsed Time: 121.687 seconds (Warm-up)
 #               62.2147 seconds (Sampling)
 #               183.901 seconds (Total)
...
aalm_tab <- xtable::xtable(tab_demo_lm, caption = "Results of separate linear regressions of demographic factors on PAQ scores for each subscale.")
...
```{r}
#make new theme that is theme_base but with 90 degree axis labels
new_theme <- function (base_size = 16, base_family = "") 
{
...
```{r}
#What do the frequencies look like?
...
levels = c("AA", "EL", "RR"),
labels = c("Affection and Attachment Behaviors", "Early Learning Behaviors", "Rules and Respect Behaviors"))
```
...
ggplot(filter(all, !is.na(category_bev)), aes(x= rating_bev, y = rating_paq, colour = category_paq)) +
  ...
  geom_jitter(aes(color = category_paq), height = .02, width = 0, alpha= .5) +
  # geom_point(alpha = .3) + 
  theme_base()+
...
save(rr_behave_mod, file = "saved_mods/rr_behave_mod.Rdata")
```
...
t_acc_q_type$p.value
```

The average accuracy for control questions was `r signif(ms$mean[ms$q_type == "con"], digits = 2)`(CI = `signif(ms$ci_lower[ms$q_type == "con"], digits = 2)`:`signif(ms$ci_upper[ms$q_type == "con"], digits = 2)`) and the average accuracy for experimenter questions was `r signif(ms$mean[ms$q_type == "exp"], digits = 2)`(CI = `signif(ms$ci_lower[ms$q_type == "con"], digits = 2)`:`signif(ms$ci_upper[ms$q_type == "exp"], digits = 2)`). There was no significant difference in accuracy between conditions, t = `r t_acc_q_type$statistic`, p = `r t_acc_q_type$p.value`.
...
```{r eval = TRUE}
uptake_mod <- brm(acc ~ q_type * scale(AA) +  
                q_type * scale(RR) + 
                q_type * scale(EL) +
                (q_type | workerid) +
                (1 | item), 
